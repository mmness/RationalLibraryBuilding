...
#this code uses a Point-Biserial correlation with a fdr correction method. It filters out features negatively correlated together you can adjust to what fits your data
...
binary_data <- as.numeric(tab2[, 2])

#  Point-Biserial Correlation Coefficient
ct <- t(sapply(3:ncol(tab2), function(x) {
  continuous_data <- tab2[, x]
  correlation <- cor(binary_data, continuous_data)
  p_value <- cor.test(binary_data, continuous_data)$p.value
  c(correlation, p_value)
}))
